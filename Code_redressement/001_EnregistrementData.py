...
class PolarToCartesianConverter:
    ...
    def calculate_area(self):
        if len(self.points) > 2:
            ...
            area = polygon.area
            print(f"Aire du polygone : {area:.2f} (en unités pixels²)")
            return area
        else:
            print("Pas assez de points pour calculer une aire.")
            return 0
    ...
    def save_to_csv(self):
        ...
        directory = r'C:\Users\dejon\OneDrive\master2\ORGUE\fichier_excel\FormeTetelecture'  #Chemin d'accès et nom du fichier.
        file_path = os.path.join(directory, 'FormeRectangleScanner.csv')
...
class PolarPlot:
    def __init__(self, converter, image_path, resolution_dpi):
        ...
        self.img = rotate(self.img, angle=-3, reshape = False)
        ...
        if event.inaxes:
            if event.button == 3:  #Click droit pour pouvoir zoomer à l'aude du click gauche
                if self.center is None:
                    ...
                    # Tracer une ligne bleue horizontale passant par le centre
                    self.ax_img.axhline(y=self.center[1], color='blue', linestyle='--', linewidth=1)
...
root = tk.Tk()
root.withdraw()  
image_path = r'C:\Users\dejon\OneDrive\master2\ORGUE\Scanner\TeteDeLecture.jpg'  # Chemin de l'image
